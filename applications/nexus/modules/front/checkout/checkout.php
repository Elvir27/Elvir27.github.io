...
class _checkout extends \IPS\Dispatcher\Controller
{
			...
			if ( $this->needsToLogin )
			{
				...
				if ( !$form->error )
				{
					...
					else
					{
						...
						if( !\IPS\Login::usernameIsInUse( "{$values['cm_first_name']} {$values['cm_last_name']}" ) )
						{
					...
					/* Save on invoice */
					$this->invoice->guest_data = array( 'member' => $member->changed, 'profileFields' => $profileFields, 'securityAnswers' => $securityQuestionAnswers, 'spamData' => array( 'code' => $spamCode, 'action' => $spamAction ), 'guestTransactionKey' => \IPS\Request::i()->cookie['guestTransactionKey'], 'pbr' => $postBeforeRegister ? $postBeforeRegister['secret'] : NULL );
				}
			}
	...
	public function _pay( $data )
	{
		...
		if ( $values !== FALSE )
		{
			...
			try
			{
				$transaction->auth = $gateway->auth( $transaction, $values, $maxMind, $recurrings, 'checkout' );
			}
	...
	public function webhook()
	{
		...
		foreach ( $this->invoice->transactions( NULL, array( 't_date>=?', \intval( \IPS\Request::i()->time ) ) ) as $transaction )
		{
			\IPS\Output::i()->redirect( $this->invoice->checkoutUrl()->setQueryString( array( 'do' => 'transaction', 't' => $transaction->id ) ) );
		}
		
		/* No, wait 5 seconds and then just show a pending screen */
		if ( !isset( \IPS\Request::i()->recheck ) )
		{
			sleep( 5 );
			\IPS\Output::i()->redirect( $this->invoice->checkoutUrl()->setQueryString( array( 'do' => 'webhook', 'time' => \intval( \IPS\Request::i()->time ), 'recheck' => 1 ) ) );
		}
		$checkoutStatus = 'pending';
